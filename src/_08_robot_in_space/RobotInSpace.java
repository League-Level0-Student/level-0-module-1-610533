...
public class RobotInSpace implements KeyEventDispatcher {
	...
	private void moveRobot(int keyPressed) throws InterruptedException {
		...
		// 3. If the up arrow is pressed, move the Robot up the screen. Up = 38
		if(keyPressed==38) {
			rob.setAngle(0);
		rob.microMove(1);
		}
		// 4. If the down arrow is pressed, move the Robot down. Down = 40
		if(keyPressed==40) {
			rob.setAngle(180);
			rob.microMove(1);
	}
		// 5. If the left arrow is pressed, make the Robot go left. Left = 37
		if(keyPressed==37) {
			rob.setAngle(270);
			rob.microMove(1);
	}
		// 6. If right is pressed, move the Robot right. Right = 39
		if(keyPressed==39) {
			rob.setAngle(90);
			rob.microMove(1);
	}
